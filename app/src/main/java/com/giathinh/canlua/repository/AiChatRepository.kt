...
class AiChatRepository @Inject constructor(
    ...
    suspend fun chat(
        ...
        val systemPrompt = buildSystemPrompt(profile, ricePrices, knowledgeHits, audience)
    ...
    suspend fun analyzeSeason(
        ...
        val systemPrompt = buildSeasonAnalysisPrompt(profile)
        val messages = listOf(
    ...
    private suspend fun callOnce(messages: List<ChatMessage>, context: String): Result<String> {
        ...
        return try {
            ...
            // Phòng vệ: bỏ ký tự điều khiển + cắt length tối đa 8000 chars để
            // Markwon không bị OOM/throw khi parse response quá dài.
            val sanitized = sanitizeAiContent(rawContent)
    ...
    private fun buildSeasonAnalysisPrompt(profile: Profile?): String {
...
""".trim()
    ...
    companion object {
        ...
        private const val MAX_AI_CONTENT_CHARS = 8000
...
"""
        ...
        /**
         * Làm sạch nội dung AI trước khi đẩy xuống UI:
         *  - Bỏ ký tự điều khiển (NUL, BEL, BS) có thể làm Markwon parse fail.
         *  - Bỏ ký tự Unicode replacement (U+FFFD) — dấu hiệu text bị decode lỗi.
         *  - Cắt tối đa [MAX_AI_CONTENT_CHARS] ký tự kèm suffix "…(đã cắt)" để tránh OOM
         *    và giữ chat bubble gọn.
         */
        private fun sanitizeAiContent(raw: String): String {
            val cleaned = raw
                .filter { c -> c.code != 0x00 && c.code != 0x07 && c.code != 0x08 }
                .replace("\uFFFD", "")
            return if (cleaned.length > MAX_AI_CONTENT_CHARS) {
                cleaned.substring(0, MAX_AI_CONTENT_CHARS) + "\n\n…(đã cắt)"
            } else {
                cleaned
            }
        }
