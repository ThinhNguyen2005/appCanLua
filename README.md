...
### 9. NewsFeed & Knowledge Base (Phase 2.7)
...
### 11. Tên Miền Đẹp & Android App Links (Phase 2.9 — mới nhất)
...
| **Cloud** | Firebase Auth + Firestore + WorkManager sync + Firebase Hosting |
| **Web Landing** | HTML5 + Vanilla CSS (Harvest Gold & Forest Emerald Theme, No Tailwind, No Purple) |
...
### Bước 1 — Clone
```bash
git clone https://github.com/ThinhNguyen2005/appCanLua.git
cd appCanLua
```
...
### Bước 4 — Build & Chạy thử
```bash
./gradlew :app:assembleDebug
# hoặc sử dụng lệnh nhanh trên Windows:
...
├── data/
│   ├── dao/               # Room DAOs
│   ├── database/          # AppDatabase, migrations (v1 to v9)
...
├── repository/            # Single source of truth layer (Sync, AI, News, Market)
├── ui/
│   ├── component/         # Reusable Compose components (Profile components)
...
│   └── viewmodel/         # MVVM bridges / ViewModels
├── util/                  # RiceCalculator, formatters, Premium state, TTS/STT
└── di/                    # Hilt modules
```
